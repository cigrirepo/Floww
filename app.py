...
st.sidebar.subheader("AI Prompt Customization")
...
custom_user_prompt = st.sidebar.text_area(
    "User prompt", value=default_user_prompt, height=100
)

# ── Generate Next-Level Workflow ────────────────────────────
if st.sidebar.button("Generate Next-Level Workflow"):
    # 1️⃣ Generate JSON workflow via AI using customized prompts
    system_msg = custom_system_prompt
    user_msg   = custom_user_prompt
